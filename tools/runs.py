...
async def create_run(
    ...
    if message:
        ...
        payload["data"]["attributes"][
            "message"
        ] = "Queued manually via Terraform Cloud MCP"
    ...
    success, data = await make_api_request("runs", method="POST", data=payload)
...
async def get_run_details(run_id: str) -> dict:
    ...
    # Make the API request
    success, data = await make_api_request(f"runs/{run_id}", method="GET")
...
async def force_execute_run(run_id: str) -> dict:
    ...
    success, data = await make_api_request(
        f"runs/{run_id}/actions/force-execute", method="POST"
    )

    if success:
        return {
            "status": "success",
            "message": "Run force-execution has been initiated",
        }
    else:
        return data  # Error info is already in the data dictionary
